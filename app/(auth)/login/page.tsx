...
export default function LoginPage() {
  ...
  // Back-button fix: an already-signed-in user landing on /login (e.g. via
  // stale history) goes straight back into the app instead of seeing the
  // login form again.
  useEffect(() => {
  ...
  async function onSubmit(values: LoginFormValues) {
    ...
    try {
      ...
      // replace (not push) so /login is not left in history — otherwise the
      // browser back button from /dashboard lands back on the login page.
      router.replace('/dashboard');
    } catch (error) {
